...
*   **Обработчики (Handlers):** Обработчики зависят от сервисов, а не напрямую от GORM. Реализована обработка ошибок, интеграция Swagger-аннотаций. Добавлены `AuthHandler` и `OrderHandler`. Общие функции вынесены в `common.go`.
...
*   **Логирование:** `logrus` используется для структурированного логирования во всех слоях. GORM также настроен на использование `logrus`.
*   **Конфигурация:** `godotenv` используется для загрузки переменных окружения из `.env`.
...
*   **Безопасность:** Пароли хешируются, используются JWT токены, реализована базовая авторизация по `UserID`.
...
### Запуск с помощью исполняемого файла (через скрипт)
...
Предполагается, что у вас есть скрипт `app_up.sh` в корне проекта, который собирает и запускает приложение (например, используя `go build -o your_app_name cmd/main.go` и затем выполняя собранный файл `.`/`your_app_name`).

1.  Сделайте скрипт исполняемым:
```bash
    chmod +x app_up.sh
```
    *(Примечание: Эта команда используется в Unix-подобных системах. Для Windows Power Shell или CMD может потребоваться другой способ запуска скрипта, если он написан на bash, или использовать собранный исполняемый файл напрямую, например, `.\your_app_name.exe` после сборки).*

2.  Запустите скрипт:
    ```bash
    ./app_up.sh
    ```
    *(Или запустите собранный исполняемый файл напрямую, если скрипт только собирает его: `./название_собранного_файла`)*
