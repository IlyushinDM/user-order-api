...
func NewOrderHandler(orderService services.OrderService, log *logrus.Logger) *OrderHandler {
...
func (h *OrderHandler) GetOrderByID(c *gin.Context) {
	...
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			...
			c.JSON(http.StatusNotFound, ErrorResponse{Error: "Order not found or access denied"})
		} else {
			h.log.WithError(err).Errorf("GetOrderByID: Failed to get order %d for user %d", orderID, authUserID)
			c.JSON(http.StatusInternalServerError, ErrorResponse{Error: "Failed to retrieve order"})
		}
		return
	}
...
func (h *OrderHandler) GetAllOrdersByUser(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, ErrorResponse{Error: "Failed to retrieve orders"})
		return
	}
...
// @Failure 400 {object} ErrorResponse "Invalid input data or order ID format"
// @Failure 401 {object} ErrorResponse "Unauthorized"
// @Failure 404 {object} ErrorResponse "Order not found or access denied"
// @Failure 500 {object} ErrorResponse "Internal server error"
// @Security BearerAuth
