...
// 		ProductName: stringPtr("Updated Product"),
// 		Quantity:    intPtr(20),
// 		Price:       floatPtr(300.00),
// 	}

// 	// Проверяем, что поля структуры доступны и имеют ожидаемые значения
// 	assert.Equal(t, stringPtr("Updated Product"), request.ProductName, "Поле ProductName должно быть доступно и иметь правильное значение")
// 	assert.Equal(t, intPtr(20), request.Quantity, "Поле Quantity должно быть доступно и иметь правильное значение")
// 	assert.Equal(t, floatPtr(300.00), request.Price, "Поле Price должно быть доступно и иметь правильное значение")

// 	// Проверяем случай с частичным обновлением
// 	partialRequest := order_model.UpdateOrderRequest{
// 		Quantity: intPtr(25), // Обновляем только количество
// 	}
// 	assert.Equal(t, "", partialRequest.ProductName, "Поле ProductName должно быть пустым при частичном обновлении")
// 	assert.Equal(t, intPtr(25), partialRequest.Quantity, "Поле Quantity должно быть доступно и иметь правильное значение при частичном обновлении")
