...
type OrderRepository interface {
	...
	GetByID(ctx context.Context, id uint, userID uint) (*models.Order, error) // Ensure user owns the order
	GetAllByUser(ctx context.Context, userID uint, page, limit int) ([]models.Order, int64, error)
}
...
func (r *gormOrderRepository) Update(ctx context.Context, order *models.Order) error {
	...
	result := r.db.WithContext(ctx).Model(&models.Order{}).Where("id = ? AND user_id = ?", order.ID, order.UserID).
	...
	if result.RowsAffected == 0 {
		...
		var exists int64
		r.db.WithContext(ctx).Model(&models.Order{}).Where("id = ?", order.ID).Count(&exists)
		if exists == 0 {
...
func (r *gormOrderRepository) Delete(ctx context.Context, id uint, userID uint) error {
	...
	result := r.db.WithContext(ctx).Where("user_id = ?", userID).Delete(&models.Order{}, id)
	if result.Error != nil {
	...
	if result.RowsAffected == 0 {
		...
		var exists int64
		r.db.WithContext(ctx).Model(&models.Order{}).Where("id = ?", id).Count(&exists)
		if exists == 0 {
...
func (r *gormOrderRepository) GetByID(ctx context.Context, id uint, userID uint) (*models.Order, error) {
...
func (r *gormOrderRepository) GetAllByUser(ctx context.Context, userID uint, page, limit int) ([]models.Order, int64, error) {
